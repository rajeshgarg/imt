# module Imt
  class UploadImagesController < ApplicationController
    ...
    # response.headers["Content-type"] = painting1.mime_type
      ...
      format.js  #  { render :html=>"<%= image_tag show_blob_image_upload_images_path(:params=>@params)  %> " }#  { render :text =>  painting1.to_blob   }  
    end     
    ...
    def flip
      ...
      respond_to do |format|  
        format.html {render :partial => "preview", :layout => false }  
        format.js { render :js=>"<%= image_tag show_blob_image_upload_images_path(@params) %> "} #  { render :text =>  painting1.to_blob   }  
      end  
    end
    def save_it
       image = get_image
    ...
    def flop
      ...
      respond_to do |format|  
        format.html {render :partial => "preview", :layout => false }  
        format.js { render :html=>"<%= image_tag show_blob_image_upload_images_path(@params) %> " } #  { render :text =>  painting1.to_blob   }  
      end  
    end
    def rotate
      @params=[params[:x],params[:y], params[:w], params[:h], params[:r]]
